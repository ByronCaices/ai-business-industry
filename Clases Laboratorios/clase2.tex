  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.47\textwidth}
      \begin{alertblock}{\faCode\enspace NoSQL --- Documento JSON}
        {\footnotesize\ttfamily\selectfont
          \textbf{\{}\\
          \quad ``id'': 1,\\
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.42\textwidth}
      \begin{conceptbox}{\faChartLine\enspace De dato a decisión}
        \small
        \textbf{Datos brutos} (\faDatabase)\\[2pt]
        $\downarrow$ Limpieza y preparación\\[2pt]
        \textbf{Datos limpios}\\[2pt]
        $\downarrow$ Análisis + Modelo de IA\\[2pt]
        \textbf{Insights accionables}\\[2pt]
        $\downarrow$\\[2pt]
...
% ─────────────────────────────────────────────────────────────
%  9a. NUESTRO CASO DE HOY — SITUACIÓN
% ─────────────────────────────────────────────────────────────
\begin{frame}{Nuestro Caso de Hoy: Planta de Manufactura}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \small
      \textbf{Situación:} Somos analistas de una planta industrial. El equipo de calidad nos entrega un CSV con \textbf{1.000 registros} de defectos de manufactura extraídos de datos reales de Kaggle.

      \vspace{0.12cm}
      \textbf{El problema:} los datos tienen errores que impiden usarlos directamente.

      \vspace{0.12cm}
      \textbf{Nuestro objetivo:}
      \begin{itemize}\setlength\itemsep{2pt}
        \item Detectar y corregir los problemas de calidad del dataset.
        \item Calcular el \textbf{costo promedio de reparación} por tipo y severidad.
        \item Identificar \textbf{qué tipo de defecto es más costoso} y qué método de inspección lo detecta mejor.
        \item Exportar un CSV limpio y gráficos para gerencia.
      \end{itemize}
    \end{column}
    \begin{column}{0.42\textwidth}
      \begin{alertblock}{\faIndustry\enspace Contexto}
        \small
        Un sistema real de control de calidad genera \textbf{miles de registros}: tipos de defecto, severidad, métodos de inspección, costos. Sin limpieza, este volumen es \textbf{inutilizable} para la IA.
      \end{alertblock}
      \vspace{0.15cm}
      \begin{conceptbox}{\faLightbulb\enspace Pregunta de negocio}
        \small
        Al finalizar responderemos:\\[4pt]
        \textbf{¿Qué tipo de defecto genera mayor costo de reparación y qué método de inspección lo detecta mejor?}
      \end{conceptbox}
    \end{column}
  \end{columns}
\end{frame}
...
% ─────────────────────────────────────────────────────────────
%  9b. NUESTRO CASO DE HOY — DATASET
% ─────────────────────────────────────────────────────────────
\begin{frame}{El Dataset: Columnas y Problemas de Calidad}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \begin{conceptbox}{\faFileCsv\enspace Columnas del dataset (Kaggle)}
        \tiny
        \begin{tabular}{ll}
          \tech{defect\_id} & ID único del defecto\\[3pt]
          \tech{product\_id} & ID del producto afectado\\[3pt]
          \tech{defect\_type} & Structural / Functional / Cosmetic\\[3pt]
          \tech{defect\_date} & Fecha de detección (\textit{texto})\\[3pt]
          \tech{defect\_location} & Surface / Component / Internal\\[3pt]
          \tech{severity} & Minor / Moderate / Critical o \textit{nulo}\\[3pt]
          \tech{inspection\_method} & Visual / Automated / Manual o \textit{nulo}\\[3pt]
          \tech{repair\_cost} & Costo de reparación (algunos \textit{negativos})
        \end{tabular}
      \end{conceptbox}
    \end{column}
    \begin{column}{0.44\textwidth}
      \begin{alertblock}{\faExclamationTriangle\enspace Problemas detectados en los datos}
        \small
        \begin{itemize}\setlength\itemsep{6pt}
          \item Valores \textbf{nulos} en \texttt{severity} (\textasciitilde10\%) e \texttt{inspection\_method} (\textasciitilde8\%)
          \item \texttt{defect\_date} guardada como \textbf{texto}, no como tipo fecha
          \item \texttt{repair\_cost} con valores \textbf{negativos} (\textasciitilde3\%) --- error de ingreso
        \end{itemize}
      \end{alertblock}
    \end{column}
  \end{columns}
...
\begin{frame}{Hoy en el Laboratorio: Hoja de Ruta}
    ...
    \begin{column}{0.48\textwidth}
      \begin{conceptbox}{\faFileAlt\enspace Entregable}
        ...
        \begin{itemize}\setlength\itemsep{4pt}
          ...
          \item Gráficos de distribución y costo (por tipo y severidad)
          \item Gráfico de tendencia mensual de defectos
          \item Gráfico \textbf{respuesta} a la pregunta de negocio (costo por tipo + efectividad por método)
        \end{itemize}
      \end{conceptbox}
    \end{column}
  \end{columns}
  ...
  \begin{alertblock}{\faLightbulb\enspace Decisión final}
    \centering\small
    Al terminar, responderemos: \textbf{¿Qué tipo de defecto genera mayor costo de reparación, y qué método de inspección es más efectivo para detectarlo?}
  \end{alertblock}
